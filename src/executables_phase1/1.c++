...
unordered_map<string, string> opcode_map = {
    ...
    {"sw", "0100011"}, {"beq", "1100011"}, {"bne", "1100011"}, {"jal", "1101111"},
    {"sh", "0100011"}, {"lui", "0110111"}, {"auipc", "0010111"},
    {"sb", "0100011"}, {"jalr", "1100111"}, {"bge", "1100011"}, {"blt", "1100011"},
    {"lb", "0000011"}, {"lh", "0000011"}, {"ld", "0000011"}, {"sd", "0100011"}
};
...
unordered_map<string, string> funct3_map = {
    ...
    {"addi", "000"}, {"andi", "111"}, {"ori", "110"}, {"lw", "010"},
    {"lb", "000"}, {"lh", "001"}, {"sw", "010"}, {"beq", "000"},
    {"bne", "001"}, {"jal", "NULL"}, {"sh", "001"}, {"lui", "NULL"},
    {"auipc", "NULL"}, {"sb", "000"}, {"jalr", "000"}, {"bge", "101"},
    {"blt", "100"}, {"ld", "011"}, {"sd", "011"}
};
...
string convert_to_machine_code(vector<string> tokens, int current_pc) {
    ...
    if (type == "R") {  // R-format
        ...
        if (tokens[0] == "lw" || tokens[0] == "lh" || tokens[0] == "ld" || tokens[0] == "lb") {
            ...
            // Parse immediate using base 10 (or change to base 0 if hexadecimal values expected)
            imm = bitset<12>(stoi(tokens[2], nullptr, 0)).to_string();
            ...
            rd = register_map[tokens[1]];
            rs1 = register_map[tokens[2]];
            imm = bitset<12>(stoi(tokens[3], nullptr, 0)).to_string();
            cout << imm + rs1 + funct3 + rd + opcode << endl;
            return imm + rs1 + funct3 + rd + opcode;
        }
    }
        ...
        rd = register_map[tokens[1]];
        // Parse the immediate using base 0 so that hexadecimal immediates are handled correctly.
        int imm_value = stoi(tokens[2], nullptr, 0);
        // For U-type instructions, shift left by 12 bits.
        imm_value = imm_value << 12;
        // Convert to a 32-bit binary string.
        string imm_full = bitset<32>(imm_value).to_string();
        // Extract the upper 20 bits.
        string imm_upper = imm_full.substr(0, 20);
